...
- 또한 매 단계 시작점으로부터 거리가 가장 짧은 정점을 우선적으로 선택하여 최단 거리를 확정한다. 이때 가중치는 반드시 양수이기 때문에 , 한 번 최단 거리로 확정된 정점은 이후 다른 경로를 통해 절대로 더 작은 값으로 갱신될 수 없다. 한번 확정된 노드는 변경되지않는다.
...
- 따라서 다익스트라 알고리즘을 사용하기 위해서는 정점 사이를 잇는 간선의 가중치가 **양수**여야 한다. 그래야 최단 거리가 확정된 정점에 대해서는 값을 업데이트하지 않아도 된다. 즉, 한번 최단거리를 확정하면 그 선택이 번복되거나 새로운 최단거리로 갱신되지않는다. 

### 다익스트라 알고리즘과 DFS
- 다익스트라 알고리즘은 간선에 양수 가중치가 있을 때 사용하며 이 가중치를 합산하여 최단 거릴ㄹ 계산한다. 
- DFS는 간선에 가중치가 없는(또는 모든 가중치가 1인) 그래프에서 주로 사용된다. 단순하게 노드들을 방문하는 것이 목적이므로, 경로의 길이를 계산하지 않는다. 만약 최단 경로를 구하더라도, 이는 단순히 경유하는 노드의 개수를 의미하며, 모든 경로를 탐색해야 하므로 효율적이지 않다.
- 즉, 다익스트라 알고리즘은 가장 짧게 갈 수 있는 가에 초점을 맞춘 알고리즘이고 DFS는 어디든 갈 수 있는가에 초점을 맞춘 알고리즘으로 최단 경로 vs 전체 탐색의 차이가 있다. 
...
- 배열의 처음 인덱스(0 또는 1)부터 마지막 인덱스(N−1 또는 N)까지 순서대로 모든 노드의 현재 거리 값을 확인한다. 이 중 아직 방문하지 않은 노드이면서 가장 작은 거리 값을 갖는 노드를 찾는다. 이런 방식은 배열의 원소를 하나씩 차례대로 검사하는 순차 탐색의 전형적인 방법이다. 
- 따라서 노드 개수가 N이라고 할 때 각 노드마다 최소 거리값을 갖는 노드를 선택해야 하는 순차 탐색이 수행되므로 (N−1)×N=O(N^2)의 시간이 걸린다. 이유는 다음과 같다.
    ...
    - 총 시간 복잡도: N번의 반복 (외부 루프) × 각 반복마다 O(N)의 순차 탐색 (내부 작업) O(N×N)=O(N^2) 의 시간 복잡도가 발생하게 된다.
...
- 4번 노드와 연결된 3,5노드를 들고 오는데 이때, 연결된 노드의 거리 값 + 연결된 노드에 오기까지의 거리가 현재 노드의 거리보다 작아야만 우선순위 큐에 저장한다. 이를 자세히 설명하면 다음과 같다. 
