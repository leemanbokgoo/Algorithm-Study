...
- 갈 수 있는 만큼 계속해서 탐색하다가 갈 수 없게 되면 다른 방향으로 다시 탐색하는 구조다. 동작 흐름은 다음과 같다.
    ...
    - 만약 끝에 도달했다면 리턴한다. 
- DFS는 경로 탐색이기떄문에 끝까지 가야 이 경로가 잘못된건지 정답인지 알수 있다 . (가지치기 시에는 끝까지 안가도 알 수있긴함.)
여튼 간에 경로 탐색이라 경로를 계속 들어가는 것이다. 조건을 만족하면 해당 경로를 결과 값에 더한다. 
...
- 주로 제약 충족 문제를 풀 때 쓰이며 DFS 방식으로 주로 구현된다. 
