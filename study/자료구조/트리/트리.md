# 트리(tree)
- 계층형 구조를 시뮬레이션 하는 추상 자료형(ADT)으로 루트 값과 부모-자식 관계의 서브 트리로 구성되며 서로 연결된 노드의 집합이다.
...
- 왼쪽 서브 트리-> 현재 노드 -> 오른쪽 서브 트리 순으로 순회한다.
    ...
    - root 노드 방문
    - 오른쪽 서브 트리 중위 순회
- 내림 차순 으로 값을 가져오기 위해서는 역순(오른쪽-> root ->왼쪽)으로 순회한다. 
...
- 트리를 삭제하는 데 주로 사용된다. 이유는 부모 노드를 삭제하기 전에 자식 노드를 삭제 하는 순으로 노드를 삭제해야 하기 때문이다.
...
![image](https://github.com/user-attachments/assets/ffbcdd0c-a29f-4721-aa9f-aa83cecea387)
