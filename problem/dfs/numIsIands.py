    ...
    for i in range(len(grid)):
        ...
        for j in range(len(grid[0])):
            if grid[i][j] == '1':
                ...
                # dfs는 중첩 함수이기때문에 부모 함수의 변수를 자유롭게 읽을 수 있다. (중첩 함수 : 함수 내에 위치한 또 다른 함수)
                ...
                # 이는 list가 가변 객체라서 가능한 일이고 불변 객체인 문자열에서는 재할당이 되지않는다. 재할당된 값은 부모 함수에선 반영되지않고, 새로운 로컬변수로 선언되며 참조 ID가 변경되어버린다.
                dfs(i, j)
                count += 1
    return count
