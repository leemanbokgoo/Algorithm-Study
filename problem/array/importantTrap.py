...
def trap2(hegiht : List[int]) -> int :
    ...
    for i in range(len(hegiht)):
        ...
        while stack and hegiht[i] > hegiht[stack[-1]]:
            ...
            # i : 현재 막대 인덱스 (오른쪽 막대)
            # stack[-1] : 왼쪽 막대 (스택에서 꺼내고 남아있는 top)
            ...
            # - hegiht[top]하는 이유는 왼쪽 막대와 오른쪽 막대 사이에 있는 중간 막대들이 길이를 빼는 것.
            # 그 중간에 있는 막대 만큼의 공간에는 물을 넣을 수 없음으로 계산에 들어가면 안된다.
            # 다만 중간 막대가 여러개일 경우에는 while stack and hegiht[i] > hegiht[stack[-1]] 조건이 true이기때문에
            # stack이 다 비워질때까지 반복문이 돌아가면서 - hegiht[top]을 한다.
            # 이걸 이해하고 싶을때는 물이 세로로 채워진다고 생각하지말고 가로로 채워진다고 생각하고 그림을 그려보면 이해가 쉽다.
            # [5, 2, 0, 0, 3, 6] 일때 물이 4, 3, 8 이렇게 채워진다. 여기서 가로로 채워진다고 생각하면, 2,3 막대의 2칸은 4,
            # 3,5 막대의 폭은 3인데 3*3이 아니라 1*3을 해야하는 이유는 이미 2 막대 길이만큼의 물은 위에서 채웠으니까 3-2 해서 1인것.
            # 그다음은 5,6인데 위에서 3 막대길이 만큼은 채웠으니까 5-3 = 2 거기에 5와 6 사이의 가로길이를 곱하면 2* 4하면 = 8
